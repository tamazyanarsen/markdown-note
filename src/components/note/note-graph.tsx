 ...
 *
 * То же, что страница /graph, только вырезка на два шага и без слоёв:
 ...
 * окрестность же считается по всему графу владельца, и платить за неё
 * при каждом открытии редактора незачем.
 */
      ...
      {open && (
        <div className="mt-2">
          {failed ? (
            ...
            <p className="text-xs text-muted-foreground">
              У этой заметки пока нет связей. Сошлись на другую заметку через{" "}
              <code>[[</code> — и она появится здесь.
            </p>
          ) : (
            <GraphCanvas
              nodes={graph.nodes}
